...
def apply_filters() -> None:
    ...
    try:
        ...
        if pause_after_filters:
            response = input("These are your configured search results and filters.\n"
                     "It's safe to change them while this dialog is open.\n"
                     "Any changes later could result in errors.\n"
                     "Type 'off' to turn off Pause after search, or press Enter to continue: ")
...
def get_page_info() -> tuple[WebElement | None, int | None]:
    ...
    try:
        ...
        current_page = int(pagination_element.find_element(
            By.XPATH, "//li[contains(@class, 'active')]").text)
    except Exception as e:
...
def get_job_main_details(job: WebElement, blacklisted_companies: set, rejected_jobs: set) -> tuple[str, str, str, str, str, bool]:
    ...
    * skip: A boolean flag to skip this job
    ...
    work_style = work_location[work_location.rfind(
        '(')+1:work_location.rfind(')')]
                    ...
                    for phrase in possible_answer_phrases:
                        for option in optionsText:
                            if phrase in option:
                                ...
                                answer = f'Decline ({option})' if len(
                                    possible_answer_phrases) > 1 else option
                                foundOption = True
                                break
                        if foundOption:
                            break
...
def external_apply(pagination_element: WebElement, job_id: str, job_link: str, resume: str, date_listed, application_link: str, screenshot_name: str) -> tuple[bool, str, int]:
    ...
    try:
        ...
        failed_job(job_id, job_link, resume, date_listed,
                   "Probably didn't find Apply button or unable to switch tabs.", e, application_link, screenshot_name)
        global failed_count
        failed_count += 1
        return True, application_link, tabs_count
...
def failed_job(job_id: str, job_link: str, resume: str, date_listed, error: str, exception: Exception, application_link: str, screenshot_name: str) -> None:
    ...
    try:
        with open(failed_file_name, 'a', newline='', encoding='utf-8') as file:
            ...
            if file.tell() == 0:
                writer.writeheader()
            writer.writerow({'Job ID': job_id, 'Job Link': job_link, 'Resume Tried': resume, 'Date listed': date_listed, 'Date Tried': datetime.now(
            ), 'Assumed Reason': error, 'Stack Trace': exception, 'External Job link': application_link, 'Screenshot Name': screenshot_name})
            file.close()
    except Exception as e:
        print_lg("Failed to update failed jobs list!", e)
        Notifier.notify("Failed to update the Excel of failed jobs!\n"
        "Possible reasons:\n"
        "1. The file is open or in use.\n"
        "2. Permission denied.\n"
        "3. File not found.",
...
def screenshot(driver: WebDriver, job_id: str, failedAt: str) -> str:
    ...
    '''
    screenshot_name = "{} - {} - {}.png".format(
        job_id, failedAt, str(datetime.now()))
...
def submitted_jobs(job_id: str, title: str, company: str, work_location: str, work_style: str, description: str, experience_required: int | Literal['Unknown', 'Error in extraction'],
    ...
    try:
        ...
        Notifier.notify("Failed to update the Excel of applied jobs!\n"
        "Possible reasons:\n"
        "1. The file is open or in use.\n"
        "2. Permission denied.\n"
        "3. File not found.",
            ...
            while current_count < switch_number:
                ...
                for job in job_listings:
                    if keep_screen_awake:
                        subprocess.run(["xdotool", "mousemove_relative", "0", "0"])  # Simulates tiny mouse movement
                    ...
                    try:
                        ...
                        failed_job(job_id, job_link, resume, date_listed,
                                   "Found Blacklisted words in About Company", e, "Skipped", screenshot_name)
                        skip_count += 1
                        continue
                    except Exception as e:
                    ...
                    if skip:
                        print_lg(message)
                        failed_job(job_id, job_link, resume, date_listed,
                                   reason, message, "Skipped", screenshot_name)
                        rejected_jobs.add(job_id)
                        skip_count += 1
                        continue
                        ...
                        try:
                            try:
                                ...
                                while next_button:
                                    ...
                                    if next_counter >= 15:
                                        if pause_at_failed_question:
                                            screenshot(
                                                driver, job_id, "Needed manual intervention for failed question")
                                            Notifier.notify("Couldn't answer one or more questions.\n"
                                ...
                                if errored != "stuck" and cur_pause_before_submit:
                                    user_response = Notifier.notify(
                                        "You submitted the application, didn't you? 😒",
                                        title="Failed to find Submit Application!",
                                        activate="com.apple.Terminal"
                                    )
                                    
                                    # Fallback for environments where Notifier may not work
                                    user_response = input("You submitted the application, didn't you? (Yes/No): ").strip()
                                    ...
                                    if user_response.lower() == "yes":
                                        ...
                                        # if screenshot_name == "Not Available":
                                        #     screenshot_name = screenshot(driver, job_id, "Failed to click Submit application")
                                        # else:
                                        #     screenshot_name = [screenshot_name, screenshot(driver, job_id, "Failed to click Submit application")]
                            ...
                            failed_job(job_id, job_link, resume, date_listed,
                                       "Problem in Easy Applying", e, application_link, screenshot_name)
                            failed_count += 1
                            discard_job()
                            continue
                    else:
...
def main() -> None:
    try:
        ...
        if not os.path.exists(default_resume_path):
            message = (f"Your default resume '{default_resume_path}' is missing!\n"
                    "Update its folder path in config.py OR\n"
                    "Add a resume with the exact name and path (check spelling and case sensitivity).\n\n"
                    "For now, the bot will continue using your previous upload from LinkedIn!")
            
            # Send macOS notification
            Notifier.notify(message, title="Missing Resume")
    
            # Fallback for CLI environments
            print("⚠️ Missing Resume Warning:\n", message)

            useNewResume = False  # Maintain original logic
        ...
        Notifier.notify(str(e), title=alert_title)
    finally:
        ...
        Notifier.notify(msg, title="Exiting..")
        print_lg(msg, "Closing the browser...")
        if tabs_count >= 10:
            msg = "NOTE: IF YOU HAVE MORE THAN 10 TABS OPENED, PLEASE CLOSE OR BOOKMARK THEM!\n\nOr it's highly likely that application will just open browser and not do anything next time!"
            Notifier.notify(msg, title="Info")
            print_lg("\n"+msg)
