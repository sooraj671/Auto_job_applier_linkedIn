...
def mac_alert(message, title="Alert"):
    """Display a macOS native alert dialog."""
    script = f'display dialog "{message}" with title "{title}" buttons ["OK"]'
    subprocess.run(["osascript", "-e", script])

def mac_confirm(message, title="Confirm", options=("Option 1", "Option 2")):
    """Display a macOS native confirmation dialog with multiple options."""
    buttons = ", ".join([f'"{opt}"' for opt in options])
    script = f'display dialog "{message}" with title "{title}" buttons [{buttons}] default button "{options[0]}"'
    result = subprocess.run(["osascript", "-e", script], capture_output=True, text=True)
    return result.stdout.strip().split(":")[-1].strip()  # Extract selected option
...
def apply_filters() -> None:
    ...
    try:
        ...
        if pause_after_filters and mac_confirm(
        "These are your configured search results and filters. It is safe to change them while this dialog is open, any changes later could result in errors and skipping this search run.",
        "Please check your results",
        ["Turn off Pause after search", "Looks good, Continue"]) == "Turn off Pause after search":
...
def submitted_jobs(job_id: str, title: str, company: str, work_location: str, work_style: str, description: str, experience_required: int | Literal['Unknown', 'Error in extraction'], 
    ...
    try:
        ...
        mac_alert("Failed to update the Excel of applied jobs!\n\nPossible reasons:\n1. The file is currently open or in use by another program\n2. Permission denied to write to the file\n3. Failed to find the file", "Failed Logging")
        ...
        try:
            while current_count < switch_number:
                ...
                for job in job_listings:
                    # Keep screen awake replacement (macOS equivalent)
                    if keep_screen_awake:
                        subprocess.run(["osascript", "-e", 'tell application "System Events" to key code 60'])  # Simulating Shift press
                    ...
                    if try_xp(driver, ".//button[contains(@class,'jobs-apply-button') and contains(@class, 'artdeco-button--3') and contains(@aria-label, 'Easy')]"):
                        try: 
                            try:
                                ...
                                while next_button:
                                    ...
                                    if next_counter >= 15: 
                                        if pause_at_failed_question:
                                            ...
                                            # Replacing pyautogui alert for failed questions
                                            mac_alert(
                                ...
                                # Handling confirmation for job application submission
                                decision = mac_confirm(
                                ...
                                # Handling confirmation for submit failure
                                if errored != "stuck" and cur_pause_before_submit and mac_confirm(
...
def main() -> None:
    try:
        ...
        # Checking if the default resume exists
        if not os.path.exists(default_resume_path):
            mac_alert(
                f'Your default resume "{default_resume_path}" is missing!\n\nPlease update its folder path "default_resume_path" in config.py\n\nOR\n\nAdd a resume with the exact name and path (check for spelling mistakes including cases).\n\n\nFor now, the bot will continue using your previous upload from LinkedIn!',
                "Missing Resume"
            )
            useNewResume = False
        ...
        mac_alert(str(e), alert_title)  # Equivalent to pyautogui.alert(e, alert_title)
    finally:
        ...
        mac_alert(msg, "Exiting..")  # Equivalent to pyautogui.alert(msg, "Exiting..")
        print_lg(msg,"Closing the browser...")
        if tabs_count >= 10:
            msg = "NOTE: IF YOU HAVE MORE THAN 10 TABS OPENED, PLEASE CLOSE OR BOOKMARK THEM!\n\nOr it's highly likely that application will just open browser and not do anything next time!" 
            mac_alert(msg, "Info")  # Equivalent to pyautogui.alert(msg, "Info")
            print_lg("\n"+msg)
